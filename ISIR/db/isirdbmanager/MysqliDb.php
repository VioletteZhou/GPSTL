...
class MysqliDb
{
    ...
    public function __construct($host = null, $username = null, $password = null, $db = null, $port = null, $charset = 'utf8', $socket = null)
    {
        echo "*******";
        //$this->$status = '';
        //$this->createdb($host, $username, $password,$db);
    ...
    *@param string $dbname
    */
    public static function createdb($host, $username, $password,$dbname)
    {
      ...
      {
        $status = "<p>Connection to $dbname failed: " . $conn->connect_error.'</p>';
      ...
      {
        $status .= "<p>Deleted database ($dbname) successfully</p>";
      }
      else
      {
        $status .= "<p>Error deleting database ($dbname) : " . mysqli_error($conn)."</p>";
      }
      ...
      {
        $status .= "<p>Created database ($dbname) successfully</p>";
      }
      else
      {
        $status .= "<p>Error creating database ($dbname) : " . $conn->error."</p>";
      }
    ...
    */
    public static function createTable($tablename,$attr)
    {
      $query = 'CREATE TABLE'. $tablename.'(';
      $cpt = 0;
      foreach($attr->entries as $row){
    ...
    function importfromcsv($file_path,$tablename)
    {
      /*$fileResource = fopen($filePath, 'r');
      $attr = explode(";",fgets($fileResource));

      while(!feof($fileResource))
      {
        $data = Array();
        $csvdata = explode(";",fgets($fileResource));

        if(count($csvdata) == count($attr)){
          for($i = 0; $i < count($csvdata);$i++){
            $data[$attr[$i]] = $csvdata[$i];
          }
          $this->_buildInsert($tablename,$data,'INSERT');
        }else{
          return false;
        }
      }
